const statusEl=document.querySelector('#status'), button=document.querySelector('#ar-button');

async function loadThreeModules(){
  let lastError;
  for(const host of ['https://cdn.jsdelivr.net/npm/three@0.180.0','https://unpkg.com/three@0.180.0']){
    try{
      const [THREE,{GLTFLoader},{OrbitControls}]=await Promise.all([
        import(`${host}/build/three.module.js`),
        import(`${host}/examples/jsm/loaders/GLTFLoader.js`),
        import(`${host}/examples/jsm/controls/OrbitControls.js`)
      ]);
      return {THREE,GLTFLoader,OrbitControls};
    }catch(error){lastError=error;console.warn(`Gagal memuat Three.js dari ${host}`,error);}
  }
  throw lastError||new Error('CDN Three.js tidak dapat diakses');
}

loadThreeModules().then(({THREE,GLTFLoader,OrbitControls})=>initializeDemo(THREE,GLTFLoader,OrbitControls)).catch(error=>{
  console.error(error);
  button.disabled=true;
  button.textContent='Three.js gagal dimuat';
  statusEl.textContent='Tidak dapat mengunduh modul Three.js dari CDN. Periksa koneksi internet atau akses CDN.';
});

function initializeDemo(THREE,GLTFLoader,OrbitControls){
