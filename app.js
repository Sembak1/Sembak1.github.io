...
let model=null, placed=null, session=null, hitSource=null;
const say=(s)=>statusEl.textContent=s;

new GLTFLoader().load('./assets/meja-mini.glb',g=>{model=g.scene;scene.add(model);say('Pratinjau siap. Seret untuk memutar atau mulai AR.');checkSupport();},undefined,e=>{console.error(e);say('Model gagal dimuat. Pastikan folder assets ikut diunggah.');button.textContent='Model tidak tersedia';});
...
async function checkSupport(){
  ...
  try{const ok=await navigator.xr.isSessionSupported('immersive-ar');button.disabled=!ok;button.textContent=ok?'Mulai AR':'Perangkat belum mendukung WebXR AR';if(ok)say('Siap. Mulai AR, pindai lantai, lalu ketuk untuk menempatkan meja.');}
